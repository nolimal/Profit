...
for (i in 1:(nn-1)){
  ...
  dy<-sort(price,decreasing = TRUE)[1:1]
  for (k in a){
  ...
  for (l in m[length(m)]){
    ...
    am<-c(am,(budget)+kupimo*(budget))
  }
  vsota<-sum(am[length(m)])
  budget[1]<-vsota
}
tickers <- stockData$.getSymbols %>% names()
sel_tickers<-c()
...
portfolioo1<-data.frame(date2,sel_tickers,am)

write.csv(portfolioo1,"2. podatki/Portfolio1.csv")
